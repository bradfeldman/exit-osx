...
function getGravatarUrl(email: string, size: number): string {
  // MD5 hash implementation for client-side
  const normalizedEmail = email.trim().toLowerCase()

  // Use a simple hash for the URL - the actual MD5 is done via the crypto-js or we use a CDN
  // Gravatar also supports SHA256, but MD5 is simpler
  // We'll use the email directly and let the browser handle it
...
function md5(string: string): string {
  ...
  function convertToWordArray(string: string): number[] {
    ...
    const wordArray: number[] = Array(numberOfWords - 1).fill(0)
    let bytePosition = 0
    let byteCount = 0
