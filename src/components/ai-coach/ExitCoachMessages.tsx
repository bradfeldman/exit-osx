...
import type { Message } from '@/contexts/ExitCoachContext'
...
export function ExitCoachMessages({ messages, isLoading }: ExitCoachMessagesProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
      {messages.map((msg, i) => {
        // Render upgrade card for gated messages
        if (msg.role === 'upgrade') {
          return (
            <div key={i} className="flex gap-3 justify-start">
              <CoachUpgradeCard
                variant={msg.upgradeAttempt === 1 ? 'detailed' : 'compact'}
                attemptNumber={msg.upgradeAttempt ?? 1}
              />
            </div>
          )
        }
