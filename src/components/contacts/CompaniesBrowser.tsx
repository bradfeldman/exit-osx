...
export function CompaniesBrowser() {
  ...
  const fetchCompanies = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/canonical/companies?${params.toString()}`)
      if (res.ok) {
  ...
  }, [page, search, companyType, dataQuality])
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <Select value={companyType} onValueChange={(v) => { setCompanyType(v); setPage(1) }}>
                ...
                <TableRow key={company.id}>
                  <TableCell>
                    <div>
                      ...
                      {company.industryName && (
                        <p className="text-sm text-muted-foreground">{company.industryName}</p>
                      )}
                  ...
                  <TableCell>
                    <Badge variant="outline">
                      {BUYER_TYPE_LABELS[company.companyType] || company.companyType}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <Badge className={cn('text-xs', qualityColors[company.dataQuality])}>
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      {company.domains.slice(0, 2).map((d) => (
                        <div key={d.domain} className="flex items-center gap-1 text-sm">
                          <Globe className="h-3 w-3 text-muted-foreground" />
                          <span className={d.isPrimary ? 'font-medium' : ''}>{d.domain}</span>
                        </div>
                      ))}
                      {company.domains.length > 2 && (
                        <p className="text-xs text-muted-foreground">+{company.domains.length - 2} more</p>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-1">
                      ...
                      <span>{company._count.people}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <span>{company._count.dealBuyers}</span>
                  </TableCell>
                  <TableCell>
